...
# folder_id = '123484346174'
# new_file = boxClient.folder(folder_id).upload(
#     '/Users/gowtham_kalaiselvan/Documents/Aspigrow/mongopython/testappgen1.docx')
# print('File "{0}" uploaded to Box with file ID {1}'.format(
#     'testappgen1.docx', new_file.id))
# connect to MongoDB, change the << MONGODB URL >> to reflect your own connection string
client = pymongo.MongoClient(
    "mongodb+srv://superadmin:Superadmin_Aspi@aspigrow-01w1b.mongodb.net/appgen?retryWrites=true&w=majority")
db = client.appgen
...
def generate_document():
    ...
    withouttable = get_all_table_patterns(
            ...
            for cell in row.cells:
                ...
                if len(fields_in_cell) > 0:
                    for field in fields_in_cell:
                        ...
                        if field.split('.')[1] == field_name.split('#')[2]:
                            child_obj_metadata.append(field)
                        else:
                            field_list.append(field)
    ...
    if len(child_obj_metadata) > 0:
        ...
        child_wrapper_list = []
        get_condition = []
        check_whr_condition = re.findall(
            "\\$tbl\\{END:(.*?)\\}", alltext_in_tbl)
        cnd_obj_set = dict()
        main_obj = ''
        ...
        for field in child_obj_metadata:
            ...
            if child_obj not in old_child_obj_meta:
                if child_obj[1] not in child_obj_list:
                    head_obj = re.findall(
                        "\\$tbl{START:[A-Za-z]\\:(.*)", document_data)
                    braces_text = re.findall(
                        "\\$tbl\\{(.*?)\\}", alltext_in_tbl)
                    selected_string = ''
                    ...
                    if len(check_whr_condition) > 0:
                        try:
                            child_obj_check = child_wrap_obj(
                                object_name_with_field, False, [], [], cnd_obj_set[child_obj[1]])
                        except KeyError:
                            child_obj_check = child_wrap_obj(
                                object_name_with_field, False, [], [], '')
                    else:
                        child_obj_check = child_wrap_obj(
                            object_name_with_field, False, [], [], '')
                    ...
                    child_wrapper = generate_child_obj(
                        child_obj, child_wrapper_list[child_obj_list.index(child_obj[1])])
        ...
        formed_query = generate_mongodb_query(json_dic, record_id)
        print('formed_query-->', collection_name)
        queried_data = collection_name.aggregate(formed_query)
        record_data = list(queried_data)
        print('queried_data-->', record_data)
        new_data = record_data[0]
        for obj_field in new_data:
            if type(new_data[obj_field]) in (tuple, list):
                new_value = {'records': new_data[obj_field]}
...
def generate_mongodb_query(json_dic, record_id):
    ...
    if len(json_dic['parentObjWrapperList']) > 0:
        for parent_obj_list in json_dic.get('parentObjWrapperList'):
            ...
            print('parent_obj_api-->', parent_obj_api)
            obj_name = parent_obj_api[1].split(')')[0]
            field_name = parent_obj_api[0]
            field_query_list = []
...
def bind_values_doc(data_dict, doc):
    ...
    table_patterns = re.findall(
        "\\$tbl\\{START:.*?\\}(.*?)\\$tbl\\{END:.*?\\}", alltext_in_tbl.replace('\n', ' ').replace('\r', ''))
    table_pattern_list = get_all_table_patterns(
                ...
                for cell in row.cells:
                    ...
                    if len(matched_patterns) > 0:
                        if len(table_patterns) > 0 and matched_patterns[0] in table_pattern_string:
                            ...
                            for value in matched_patterns:
                                cell.text = attach_field_values(
                                    value, data_dict)
                    elif len(function_list) > 0:
                        field_value = ''
                        field_value = generate_functions(
                    ...
                    for cell in row.cells:
                        ...
                        if len(table_row_details) > 0 and table_row_details[0] not in table_fields_list:
                            ...
                            if len(function_list) > 0:
                                ...
                                for value in matched_patterns:
                                    text_in_cell = cell.text
                                    field_value = attach_field_values(
                                        value, data_dict)
                                    field_value = text_in_cell.replace(
                                        '${'+value+'}', field_value)
                                    cell.text = field_value
